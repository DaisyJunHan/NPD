...
public class GameDaoImpl implements IGameDao {
	...
	public int addGame(Game game) {
		...
		int percent = game.getPercent();
		int accuracy = game.getAccuracy();
		String strSQL = "insert into npd.game_record values(?,?,?,?,?,null,?,?,?,null,null,null)";
		int result = this.dbconn.execOther(strSQL, new Object[]{gid,time,score,level,pid,date,percent,accuracy});
		return result;
	}
	/* (non-Javadoc)
		...
		if(rs!=null){
			try {
				while(rs.next()){
					...
					game.setDate(rs.getString(7));
					game.setPercent(rs.getInt(8));
					game.setAccuracy(rs.getInt(9));
					game.setExt1(rs.getString(10));
					game.setExt2(rs.getString(11));
					game.setExt3(rs.getString(12));
					lst.add(game);
				}
